...
/**
 * Advanced alert builder — the "condition set" override reached from the score
 * alert surface's "Advanced setup" link. Alerts are always anchored to a spot
 * (passed via query params), so there's no map pin-drop here. Bare visits (no
 * spot context) redirect to /notifications, which is the manage view.
 */
function AdvancedAlertBuilder() {
  ...
  // Back to the score-alert surface with the same spot context intact.
  const scoreHref = `/profile/custom-alerts?${params.toString()}`
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      <div>
        ...
        >
          <ArrowLeft className="w-3.5 h-3.5" /> BACK TO SCORE ALERT
        </Link>
      ...
      <CustomAlertForm
        ...
        onSubmit={submit}
        onCancel={() => router.push(scoreHref)}
      />
    </div>
  )
}
