/**
 * One alert evaluation run, callable from any trigger.
 *
 * The work used to live inside the POST handler, which meant the only way to
 * run it was to be that HTTP route. Two schedulers now drive it (a Vercel cron
 * and the GitHub Action), so the body lives here and the routes are just doors
 * onto it. Nothing about the evaluation itself changed in the move.
 *
 * Safe to run concurrently. Every message is claimed in `alert_day_notices`
 * before it is sent, under a UNIQUE (alert_profile_id, target_date, beat), so a
 * second worker arriving mid-run loses the claim rather than sending a
 * duplicate. See claimBeat() in custom-alert-engine.
 */
...
export async function runAlertEvaluation(): Promise<AlertEvaluationOutcome> {
